...
namespace EFramework.Runtime.Math
{
    public static class BezierHelper
    {
            ...
            int numberOfPoints)
        {
            ...
            List<Vector3> result = new(numberOfPoints + 1);
            for (int i = 0; i <= numberOfPoints; i++)
            {
                float t = i / (float)numberOfPoints;
                result.Add(CalculateBezierPoint(t, startPoint, controlPoint, endPoint));
            }

            return result;
        }

        [Obsolete("Use CalculateBezierPoint. This method calculates a quadratic Bezier point, not a linear one.")]
        ...
        {
            if (arcLengthTable == null || arcLengthTable.Count <= 1)
            {
                return 0f;
            }
