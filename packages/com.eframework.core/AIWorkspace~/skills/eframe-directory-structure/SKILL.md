...
5. Put UI prefabs by UI type: pages in `Res/UI/Panels/<PanelName>/`, popups in `Res/UI/Popups/<PopupName>/`, reusable item prefabs in `Res/UI/Widgets/<WidgetName>/`, and cross-UI shared assets in `Res/UI/Common/...`. UI FX follows the owning UI directory or `Res/UI/Common/FX`.
6. Keep scene files and scene dependencies distinct: `.unity` files go under `Assets/Scenes/...` or `Assets/Modules/<Name>/Scenes/...`; scene-owned prefabs, FX, textures, lighting, and config go under `Res/SceneAssets/<SceneName>/...`.
...
8. Place resources by owner and reuse scope: module-private assets stay under `Assets/Modules/<Name>/Res/...`; cross-module shared assets can move to `Assets/App/Res/...`; UI-specific FX/materials stay with UI, scene/gameplay FX and scene-specific materials stay with `SceneAssets`, and broadly shared materials/shaders go to `Assets/App/Res/Materials` or `Assets/App/Res/Shaders`.
