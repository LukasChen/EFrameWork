...
$packageRootCandidate = if (Test-Path (Join-Path $frameworkRoot "AIWorkspace~")) {
    $frameworkRoot
}
else {
    Join-Path $frameworkRoot "packages\com.eframework.core"
}
$sourcePackageRoot = if (Test-Path (Join-Path $packageRootCandidate "AIWorkspace~")) {
    (Resolve-Path $packageRootCandidate).Path
}
else {
    $null
}
$sourceRoot = if ($sourcePackageRoot) { Join-Path $sourcePackageRoot "AIWorkspace~" } else { $null }
...
$managedRootBlocks = @(
    [pscustomobject]@{
        ...
        SourcePath = "AIWorkspace~/managed-blocks/eframe-codex-root.md"
    ...
    [pscustomobject]@{
        ...
        SourcePath = "AIWorkspace~/managed-blocks/eframe-copilot-root.md"
    ...
    [pscustomobject]@{
        ...
        SourcePath = "AIWorkspace~/managed-blocks/eframe-claude-root.md"
...
if (-not $sourceRoot -or -not (Test-Path $sourceRoot)) {
    throw "EFrame AI workspace source folder not found. Expected package AIWorkspace~ under $frameworkRoot or $packageRootCandidate."
}
...
function Test-ManifestEntrySelected {
    ...
    if ($relativePath -eq ".github/copilot-instructions.md") {
...
function Test-ManifestEntryIsSupportFile {
    param(
        [object]$Entry
    )
    ...
    return -not [string]::IsNullOrWhiteSpace([string]$Entry.sourcePath)
}

function Get-SelectedSupportFileEntries {
    param(
        [object[]]$Entries
    )

    return @($Entries | Where-Object { Test-ManifestEntryIsSupportFile -Entry $_ })
}
...
function Resolve-ManifestEntrySourcePath {
    ...
    if (-not [string]::IsNullOrWhiteSpace($sourcePath)) {
        return Join-Path $sourcePackageRoot ($sourcePath -replace "/", "\")
    }

    if ($relativePath -like ".github/instructions/*") {
        $localPath = $relativePath.Substring(".github/instructions/".Length)
        return Join-Path (Join-Path $sourceRoot "instructions") ($localPath -replace "/", "\")
    }

    if ($relativePath -like ".github/skills/*") {
        $localPath = $relativePath.Substring(".github/skills/".Length)
        return Join-Path (Join-Path $sourceRoot "skills") ($localPath -replace "/", "\")
    }

    return Join-Path $sourcePackageRoot ($relativePath -replace "/", "\")
}
...
function Get-ManifestFileStatus {
    ...
    foreach ($entry in $Entries) {
        ...
        if ($kind -eq "managedBlock") {
            if ($RootPath -eq $frameworkRoot) {
                $sourcePath = Resolve-ManifestEntrySourcePath -Entry $entry
        ...
        if ($RootPath -eq $frameworkRoot) {
            $absolutePath = Resolve-ManifestEntrySourcePath -Entry $entry
        }
...
function Sync-ManagedBlock {
    ...
    $sourcePath = Join-Path $sourcePackageRoot ($BlockSpec.SourcePath -replace "/", "\")
...
function Write-SyncStatusReport {
    ...
    Write-Host "Package root: $sourcePackageRoot"
...
if ($resolvedTargetRoot -eq $frameworkRoot) {
    if ($StatusOnly) {
        Write-Host "Framework AI version: $sourceVersion"
        Write-Host "Target AI version: $sourceVersion"
        Write-Host "AI workspace config is up to date. Target is the framework workspace."
        Write-SyncStatusReport -SelectedClients $selectedClients
        Write-ManifestFileIntegrityReport -RootPath $frameworkRoot -Entries $sourceManifestFileEntries | Out-Null
        return
    }

    Write-Host "Target is the framework workspace. No project sync needed."
...
foreach ($entry in (Get-SelectedSupportFileEntries -Entries $sourceManifestFileEntries)) {
    $sourcePath = Resolve-ManifestEntrySourcePath -Entry $entry
    $destinationPath = Join-Path $resolvedTargetRoot (([string]$entry.path) -replace "/", "\")
    Sync-File -SourcePath $sourcePath -DestinationPath $destinationPath -Overwrite:$Force
}
