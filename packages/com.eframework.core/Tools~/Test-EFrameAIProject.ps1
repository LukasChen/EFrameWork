...
if ($FrameworkRoot) {
    ...
    $frameworkManifestPath = Join-Path $resolvedFrameworkRoot "packages\com.eframework.core\AIWorkspace~\eframe-ai.manifest.json"
    if (-not (Test-Path $frameworkManifestPath)) {
        $frameworkManifestPath = Join-Path $resolvedFrameworkRoot "AIWorkspace~\eframe-ai.manifest.json"
    }
...
if ($targetClients.Count -gt 0) {
    if ($eframeInstructions.Count -eq 0) {
        $errors.Add("AI clients are selected, but no synced eframe-* instruction files were found.")
    }
    if ($eframeSkills.Count -eq 0) {
        $errors.Add("AI clients are selected, but no synced eframe-* skills were found.")
    }
}
