...
5. Put controller code in runtime structure, bind button/events in the controller lifecycle, and clean subscriptions on close/dispose.
...
7. Keep View wrappers thin: cache generated binding/components in `OnBindingSet()`, keep reusable business state outside the View wrapper.
...
- Confirm every event subscription, button listener, async handle, and controller-owned resource has a matching cleanup path.
