...
This file is a bridge inventory, not the final machine-readable registry. The target architecture is defined in `AI_RULE_INFORMATION_ARCHITECTURE.md`; the first structured registry draft lives at `rules/registry.yaml`, with layered references in `rules/rule-sets.yaml`, `rules/rule-views.yaml`, and task-driven loading guidance in `rules/loading-policy.yaml`.
