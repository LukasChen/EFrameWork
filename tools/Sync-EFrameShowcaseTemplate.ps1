...
if ([string]::IsNullOrWhiteSpace($SourceRoot)) {
    $SourceRoot = Join-Path $frameworkRoot "test-fixtures/EFrameShowcaseUnity/Assets/Modules/EFrameExtensionShowcase"
}

if ([string]::IsNullOrWhiteSpace($TargetRoot)) {
    $TargetRoot = Join-Path $frameworkRoot "packages/com.eframework.core/Editor/Templates/Modules/EFrameExtensionShowcase"
}

if (-not (Test-Path -LiteralPath $SourceRoot)) {
    throw "Showcase source module not found: $SourceRoot"
}
...
if ($CheckOnly) {
    ...
    if (-not (Test-Path -LiteralPath $targetFullPath)) {
        throw "Showcase template target not found: $targetFullPath"
    }
    ...
    if ($errors.Count -gt 0) {
        ...
        throw "Extension Showcase template is out of sync with the fixture source."
    }

    Write-Host "Extension Showcase template is in sync with the fixture source."
    return
}
...
Write-Host "Synced Extension Showcase template from:"
